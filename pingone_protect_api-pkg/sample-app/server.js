...
fastify.post("/getRiskEvaluation", async (req, res) => {
  ...
  try {
    ...
    res
      .code(500)
      .send(
        "An error occurred getting a worker token from PingOne application"
      );
    return;
  }
  ...
  const riskResponse = await fetch(
    `https://api.pingone.${process.env.P1_DOMAIN}/v1/environments/${process.env.P1_ENV_ID}/riskEvaluations`,
    {
      method: "POST",
      headers: {
  ...
  if (riskResponse.status !== 201) {
    res
      .code(500)
      .send("Unexpected error occurred while creating a risk evaluation");
    return;
  }
  ...
  const updateResponse = await fetch(
    `https://api.pingone.${process.env.P1_DOMAIN}/v1/environments/${process.env.P1_ENV_ID}/riskEvaluations/${riskJson.id}/event`,
    {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${workerToken}`,
  ...
  if (updateResponse.status !== 200) {
    res
      .code(500)
      .send("Unexpected error occurred while updating risk evaluation");
    return;
  }
...
async function searchPingOneForUser(username, token) {
  ...
  const response = await fetch(
    encodeURI(
      `https://api.pingone.${process.env.P1_DOMAIN}/v1/environments/${process.env.P1_ENV_ID}/users?filter=username eq \"${username}\"`
    ),
    {
      headers: {
...
async function getWorkerToken() {
  const basicAuth = btoa(
    `${process.env.P1_WORKER_CLIENT_ID}:${process.env.P1_WORKER_CLIENT_SECRET}`
  );
  const encodedBody = new URLSearchParams();
  encodedBody.append("grant_type", "client_credentials");

  const tokenResponse = await fetch(
    `https://auth.pingone.${process.env.P1_DOMAIN}/${process.env.P1_ENV_ID}/as/token`,
    {
      headers: {
